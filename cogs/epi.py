...
class GetNotifiedButton(ui.ActionRow):
    ...
    async def on_get_notified_click(self, interaction: discord.Interaction, button: ui.Button):
        if interaction.user.id not in epi_users:
            ...
            await interaction.response.send_message("You will now be notified when this issue is fixed!", ephemeral=True)
        else:
            await delete_epi_user(interaction.user.id)
            epi_users.remove(interaction.user.id)
            await interaction.response.send_message("You will no longer be notified for this issue!", ephemeral=True)
...
class epi(commands.Cog):
    ...
    def generate_epi_layout_view(self) -> GetNotifiedView:
        ...
        if self.epi_Message or self.epi_msg:
            description += "\n\n"
        description += "-# You can also always check the [Sapphire status page](https://sapph.xyz/status)"
